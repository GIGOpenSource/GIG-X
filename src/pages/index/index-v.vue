<template>
	<view class="tabs">
		...
		<!-- #ifdef MP-WEIXIN -->
		<esc-swiper :autoplay="false" :current.sync="tabIndex" :width="screenWidth" :height="screenHeight"
			:itemWidth="750" :itemHeight="screenHeight" :size="tabList.length" @change="escChange" ref="escRef">
		...
		<view class="head" :style="{ top: statusBarHeight + 'px', width: screenWidth + 'px' }">
			<view class="head-item" @click="tabChange(index)" v-for="(item, index) in tabList" :key="index">
				<text class="head-item-text" :class="tabIndex == index ? 'head-item-textActive' : ''">{{ item.name
					}}</text>
			...
			<view class="head-operate">
				...
				<span></span>
				<image src="/static/tsp-icon/home-search.png" class="head-operate-icon" @click="handleJump2Search">
				</image>
			</view>
		</view>
