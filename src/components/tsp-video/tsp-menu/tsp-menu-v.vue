<template>
	<view class="menuPage">
			...
			<view class="vodMenu" :class="[
				...
				<view class="menu-avatar">
					...
					<view class="follow" @click="followBtn(index)" v-if="!item.followReally"
						...
						<image src="/static/tsp-icon/guanzhu.png" mode="" class="follow-guanzhu"
							v-if="followShow == null || followShow == 0" :class="{ followActive: followShow == 0 }">
						</image>
					</view>
				</view>
				<!-- 点赞 -->
				<view class="fabulous">
					<view class="fabulous-image" :class="likeeMeans" @click="fabulousBtn(index)"
				...
				<view class="fabulous" :class="collectionMeans" style="margin-top: 30rpx" @click="collectionBtn(index)">
					<view class="fabulous-image" >
						...
						<view class="collection-pellet" :class="[collectionShow ? 'collection-pellet-active' : '']">
						</view>
					</view>
		...
		<!-- 旋转头像 -->
		<view class="avatarMenu" v-if="item.rotateImgShow" :class="[
			vodIndex == index
				? sliderDrag
					? 'vodMenu-bright1'
					: moveOpacity
			...
			<view class="rotate-avatar k-paused" :class="[item.vodPaly && palyCartoon ? 'k-running' : '']"
				@click="JumpBtn(4)">
